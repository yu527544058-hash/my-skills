...
第一次用，先输入 **`/aht-target help`**，会显示完整的使用说明：三个使用场景、每种需要准备什么数据、常见问答。
...
每条处理时间 = 起步时间 + 每单位增量 × 长短，就像打车费 = 起步价 + 每公里的钱。"长短"就是影响处理时间的主要因素：视频看时长，文本看字数，图片看张数。项目经理试标几十条，就能算出起步时间是多少、每单位增量多少。
...
- `--save pm.json`：把算出来的起步时间和每单位增量存成一个文件，后面的步骤要用它
